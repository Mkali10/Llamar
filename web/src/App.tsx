...
export default function App({token}:{token:string}) {
  ...
  return <div className="shell">
    ...
    <main>{active==='AI providers'&&<ProviderSettings token={token}/>} {active==='Telephony providers'&&<TelephonySettings token={token}/>} {active==='AI agents'&&<AgentStudio token={token}/>} {active==='AI routing'&&<RoutingStudio token={token}/>} {active==='Campaigns'&&<CampaignStudio token={token}/>} {active==='Live calls'&&<LiveCalls token={token}/>} {active==='Team & rights'&&<TeamSettings token={token}/>}<div className={['AI providers','Telephony providers','AI agents','AI routing','Campaigns','Live calls','Team & rights'].includes(active)?'viewHidden':''}><header><div><p>Operations / Overview</p><h1>Good evening, Ankit</h1><span>Live operational view across calling, agents and compliance.</span></div><div className="actions"><button>Export report</button><button className="primary" onClick={()=>setActive('Campaigns')}>Create campaign</button></div></header>
