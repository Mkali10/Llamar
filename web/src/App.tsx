import {useEffect,useState} from 'react';import {dashboard} from './api';import ProviderSettings from './ProviderSettings';import AgentStudio from './AgentStudio';
const nav = ['Overview', 'Campaigns', 'Live calls', 'AI agents', 'Flow builder', 'DIDs & approvals', 'Recordings', 'Reports', 'AI providers', 'Team & rights'];
const calls = [
...
export default function App({token}:{token:string}) {
  ...
  return <div className="shell">
    ...
    <main>{active==='AI providers'&&<ProviderSettings token={token}/>} {active==='AI agents'&&<AgentStudio token={token}/>}<div className={active==='AI providers'||active==='AI agents'?'viewHidden':''}><header><div><p>Operations / Overview</p><h1>Good evening, Ankit</h1><span>Live operational view across calling, agents and compliance.</span></div><div className="actions"><button>Export report</button><button className="primary">Create campaign</button></div></header>
