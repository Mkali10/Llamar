import {lazy,useEffect,useState} from 'react';import {dashboard} from './api';import ProviderSettings from './ProviderSettings';import AgentStudio from './AgentStudio';import RoutingStudio from './RoutingStudio';import CampaignStudio from './CampaignStudio';import LiveCalls from './LiveCalls';import TelephonySettings from './TelephonySettings';import TeamSettings from './TeamSettings';import DidWorkspace from './DidWorkspace';import FlowBuilder from './FlowBuilder';import Recordings from './Recordings';import Reports from './Reports';import MeetingWorkspace from './MeetingWorkspace';const AgentDesk=lazy(()=>import('./AgentDesk'));
const nav = ['Overview', 'Agent desk', 'Campaigns', 'Live calls', 'AI agents', 'AI routing', 'Flow builder', 'Meetings', 'DIDs & approvals', 'Recordings', 'Reports', 'AI providers', 'Telephony providers', 'Team & rights'];
const calls = [
...
export default function App({token}:{token:string}) {
  ...
  return <div className="shell">
    ...
    <main>{active==='AI providers'&&<ProviderSettings token={token}/>} {active==='Telephony providers'&&<TelephonySettings token={token}/>} {active==='AI agents'&&<AgentStudio token={token}/>} {active==='AI routing'&&<RoutingStudio token={token}/>} {active==='Campaigns'&&<CampaignStudio token={token}/>} {active==='Live calls'&&<LiveCalls token={token}/>} {active==='Team & rights'&&<TeamSettings token={token}/>} {active==='DIDs & approvals'&&<DidWorkspace token={token}/>} {active==='Flow builder'&&<FlowBuilder token={token}/>} {active==='Recordings'&&<Recordings token={token}/>} {active==='Reports'&&<Reports token={token}/>} {active==='Meetings'&&<MeetingWorkspace token={token}/>} {active==='Agent desk'&&<AgentDesk token={token}/>}<div className={['AI providers','Telephony providers','AI agents','AI routing','Campaigns','Live calls','Team & rights','DIDs & approvals','Flow builder','Recordings','Reports','Meetings','Agent desk'].includes(active)?'viewHidden':''}><header><div><p>Operations / Overview</p><h1>Good evening, Ankit</h1><span>Live operational view across calling, agents and compliance.</span></div><div className="actions"><button onClick={()=>setActive('Reports')}>Export report</button><button className="primary" onClick={()=>setActive('Campaigns')}>Create campaign</button></div></header>
      ...
      <section className="grid"><article className="panel calls"><div className="panelHead"><div><h2>Live call activity</h2><p>Masked customer data · realtime state</p></div><button onClick={()=>setActive('Live calls')}>Open monitor</button></div>
