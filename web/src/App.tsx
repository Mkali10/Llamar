import {useEffect,useState} from 'react';import {dashboard} from './api';import ProviderSettings from './ProviderSettings';import AgentStudio from './AgentStudio';import RoutingStudio from './RoutingStudio';
...
export default function App({token}:{token:string}) {
  ...
  return <div className="shell">
    ...
    <main>{active==='AI providers'&&<ProviderSettings token={token}/>} {active==='AI agents'&&<AgentStudio token={token}/>} {active==='AI routing'&&<RoutingStudio token={token}/>}<div className={['AI providers','AI agents','AI routing'].includes(active)?'viewHidden':''}><header><div><p>Operations / Overview</p><h1>Good evening, Ankit</h1><span>Live operational view across calling, agents and compliance.</span></div><div className="actions"><button>Export report</button><button className="primary">Create campaign</button></div></header>
